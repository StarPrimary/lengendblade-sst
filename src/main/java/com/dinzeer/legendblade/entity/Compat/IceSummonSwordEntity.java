package com.dinzeer.legendblade.entity.Compat;
...
public class IceSummonSwordEntity  extends EntityAbstractSummonedSword {
    private static final EntityDataAccessor<Boolean> IT_FIRED = SynchedEntityData.defineId(IceSummonSwordEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Float> SPEED = SynchedEntityData.defineId(IceSummonSwordEntity.class, EntityDataSerializers.FLOAT);

    private static final EntityDataAccessor<Vector3f> OFFSET = SynchedEntityData.defineId(IceSummonSwordEntity.class, EntityDataSerializers.VECTOR3);
    long fireTime = -1;

    public IceSummonSwordEntity(EntityType<? extends Projectile> entityTypeIn, Level worldIn)
    {
    ...
    public static IceSummonSwordEntity createInstance(PlayMessages.SpawnEntity packet, Level worldIn)
    {
        return new IceSummonSwordEntity(LBEntiteRegristrys.ss, worldIn);
    }
        ...
        if (!level.isClientSide()) {
            if (entity instanceof LivingEntity livingEntity) {
                EntityDataProvider.getCapability(livingEntity).ifPresent((data) -> {
                    data.frozenData.setFrozen(livingEntity, 300);
                });
                ...
                if (getOwner() instanceof LivingEntity entity1)
                    livingEntity.knockback(1.0, entity1.getX() - livingEntity.getX(), entity1.getZ() - livingEntity.getZ());
            }

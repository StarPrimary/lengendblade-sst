...
public class Lightingsum  extends SpecialEffect {
    public Lightingsum() {
    ...
    public static void onDoingSlash(SlashBladeEvent.DoSlashEvent event) {
        if(ModList.get().isLoaded(SlashBladeAddon.MODID)) {
            ...
            if (state.hasSpecialEffect(SBALBSlashArtRegsitry.Lightingsum.getId())) {
                ...
                if (SpecialEffect.isEffective(SBALBSlashArtRegsitry.Lightingsum.get(), level)) {
                    if(random.nextInt(100)>=70){
                        LightingSwords.doSlash(player,false,2.0,5F);
                    }
                }
            }
        }

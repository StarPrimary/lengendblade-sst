...
public class Poison extends SpecialEffect {
    public Poison() {
    ...
    public  static  void blaze(SlashBladeEvent.UpdateEvent event){
        ...
        if(state.hasSpecialEffect(LBSpecialEffectsRegistry.poison.getId())) {
            ...
            if(SpecialEffect.isEffective(LBSpecialEffectsRegistry.poison.get(),level)){
    ...
    public static void onSlashBladeHit(SlashBladeEvent.HitEvent event) {
        ...
        if (state.hasSpecialEffect(LBSpecialEffectsRegistry.poison.getId())) {
            ...
            int level = player.experienceLevel;
            if (SpecialEffect.isEffective(LBSpecialEffectsRegistry.poison.get(), level)) {
                event.getTarget().addEffect(new MobEffectInstance(MobEffects.POISON, 300, 3));
            }
        }
